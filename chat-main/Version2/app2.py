...
# URL для загрузки XML с товарами
PRODUCTS_XML_URL = "https://example.com/products.xml"  # Замените на реальный URL

# Хранение истории сообщений для текущей сессии
message_history = []

# Загрузка данных о товарах из XML по URL
def load_products():
    try:
        # Запрос XML файла по URL
        response = requests.get(PRODUCTS_XML_URL, timeout=10)
        response.raise_for_status()  # Проверка на ошибки HTTP
        xml_content = response.content
        tree = ET.fromstring(xml_content)  # Парсинг XML из строки
        products = []
        ...
        logging.info(f"Успешно загружено {len(products)} товаров с {PRODUCTS_XML_URL}")
        return products
    except requests.exceptions.RequestException as e:
        logging.error(f"Ошибка при загрузке XML с {PRODUCTS_XML_URL}: {e}")
        return []
...
@app.route('/')
def index():
    return render_template('chat.html')  # Рендерим страницу с чатом
